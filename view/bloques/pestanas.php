<div id="primary-menu-wrapper" class="clearfix">
	<div id="primary-menu" style="width: <?php echo ((ANCHO_PESTANA * PESTANAS_FILA + 18 
			* (PESTANAS_FILA - 1))); ?>px;">
			...
			<ul class="menu sf-menu sf-js-enabled">
				...
				<?php foreach ($menus as $menu) { ?>
					...
					<li class="<?php echo $claseMenu1; ?><?php if (!$cont) { ?> menu_rojo<?php } ?>" 
							...
							echo ANCHO_PESTANA; ?>px; <?php if (($cont % PESTANAS_FILA) == 0) { 
							?>clear: both;<?php } ?>"><a <?php if (!$cont) { ?>class="menu_rojo"<?php } ?> 
							href="<?php echo $menu->enlace(); ?>"><?php echo formato_html($menu->titulo); ?></a>
						<?php if (count($menu->hijos) > 0) { ?>
							<ul class="menu" style="display: none; visibility: hidden;  width: <?php 
									echo (ANCHO_PESTANA + 10); ?>px;">
								<?php foreach ($menu->hijos as $submenu) { ?>
									<li title="<?php echo formato_html($submenu->titulo); ?>" 
											style="background-color: #D1DFEF; width: <?php 
											echo ANCHO_PESTANA; ?>px;"><a href="<?php 
											echo $submenu->enlace(); ?>"><?php 
