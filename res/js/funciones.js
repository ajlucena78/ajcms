...
function ver_popup(ancho, alto, url, funcion, titulo)
{
	...
	if (!ancho)
		ancho = 300;
	popup.style.width = ancho + 'pt';
	if (!alto)
		alto = 200;
	popup.style.height = alto + 'pt';
	if (url)
	{
		contenido_popup.innerHTML = '';
		carga(url, 'contenido_popup', funcion);
		contenido_popup.innerHTML = '<br />';
		contenido_popup.innerHTML += '<img src="' + URL_BASE + 'res/img/web/ajax-loader.gif" alt="Cargando..." />';
		contenido_popup.innerHTML += '<br />&nbsp;<br />Cargando...';
	}
	if (titulo)
	{
		var titulo_popup = document.getElementById('titulo_popup');
		if (!titulo_popup)
			return false;
		titulo_popup.innerHTML = titulo;
	}
	capa.style.display = 'table';
}
...
{
	var win = window.open(url, 'ventana_impresora');
	win.onload = imprime;
}

function imprime()
{
	var win = window.open(null, 'ventana_impresora');
	win.close();
	parent.focus();
}

function convierte_numero(num)
{
